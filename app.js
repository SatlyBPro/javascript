...
'};'
  ...
  function setupEditor() {
    ...
    setupTouchCursorControl();
  }

  // ---------------------------------------------------------------
  // Touch cursor control (iPad/mobile "trackpad" cursor dragging)
  //
  // Monaco has no built-in touch equivalent of the native iOS text-view
  // gesture where touching-and-holding the caret (or the on-screen
  // spacebar) turns finger movement into fine cursor positioning. Without
  // this, touch users can only tap to place the cursor, which is
  // imprecise. This adds two ways to get that "trackpad" behavior:
  //
  //   1. Press and hold directly on/near the blinking caret, then drag —
  //      the cursor follows the finger 1:1 in text-position terms.
  //   2. Press and hold the on-screen "space" toolbar key, then drag
  //      anywhere on screen — same effect, without needing to land the
  //      touch exactly on the thin caret.
  //
  // Both use a short hold delay so normal taps/scrolls/selection aren't
  // hijacked, and both move the cursor by translating finger delta into
  // Monaco's own scaled "mouse target" hit-testing so it stays accurate
  // at any font size or scroll position.
  // ---------------------------------------------------------------

  function setupTouchCursorControl() {
    if (!editor) return;
    const domNode = editor.getDomNode();
    if (!domNode) return;

    const HOLD_MS = 160;      // time finger must stay still before drag mode engages
    const MOVE_CANCEL_PX = 8; // finger movement during the hold that cancels it (treated as a scroll/selection instead)
    ...
    function getCaretClientRect() {
      // Monaco doesn't expose the caret's screen rect directly, so derive
      // it from the current cursor position via the editor's own
      // coordinate conversion (accounts for scroll/line height/font size).
      const pos = editor.getPosition();
      if (!pos) return null;
      ...
      return {
        ...
        y: editorRect.top + coords.top,
        height: coords.height || 18
      };
    }

    function isNearCaret(clientX, clientY) {
      const rect = getCaretClientRect();
      if (!rect) return false;
      const padX = 14; // generous touch target around the thin caret
      const padY = rect.height / 2 + 6;
      return (
        clientX >= rect.x - padX &&
        clientX <= rect.x + padX &&
        clientY >= rect.y - padY &&
        clientY <= rect.y + rect.height + padY
      );
    }
    ...
    function moveCursorTo(clientX, clientY) {
      const pos = positionFromClientPoint(clientX, clientY);
      if (!pos) return;
      editor.setPosition(pos);
      editor.revealPositionInCenterIfOutsideViewport(pos, monacoRef.editor.ScrollType.Smooth);
    }

    function beginDrag() {
      dragActive = true;
      domNode.classList.add("touch-cursor-dragging");
      if (navigator.vibrate) { try { navigator.vibrate(8); } catch (e) {} }
    }

    function endDrag() {
      dragActive = false;
      domNode.classList.remove("touch-cursor-dragging");
    ...
    domNode.addEventListener("touchstart", function (e) {
      ...
      const t = e.touches[0];
      if (!isNearCaret(t.clientX, t.clientY)) return;
      ...
      holdTimer = setTimeout(function () {
        if (startTouchId !== null) beginDrag();
      }, HOLD_MS);
    }, { passive: true });
    ...
    domNode.addEventListener("touchmove", function (e) {
      ...
      if (!dragActive) {
        ...
        if (Math.hypot(dx, dy) > MOVE_CANCEL_PX) {
          // Finger moved before the hold engaged — this is a normal
          // scroll/selection gesture, let Monaco handle it natively.
          clearTimeout(holdTimer);
          startTouchId = null;
          return;
        }
    ...
    // -------------------------------------------------------------
    // On-screen "space" key: press and hold, then drag anywhere to move
    // the cursor, like holding down the spacebar on iOS's own keyboard
    // turns the keyboard into a trackpad. Easier to grab reliably than
    // the thin blinking caret itself.
    // -------------------------------------------------------------

    const spaceKey = document.querySelector('.mkey[data-space-trackpad]');
    if (spaceKey) {
      let spaceTouchId = null;
      ...
      function endSpaceDrag() {
        spaceDragging = false;
        spaceKey.classList.remove("active-trackpad");
      ...
      spaceKey.addEventListener("touchstart", function (e) {
        ...
        spStartX = t.clientX;
        spStartY = t.clientY;
        spaceTouchId = t.identifier;
        clearTimeout(spaceHoldTimer);
        spaceHoldTimer = setTimeout(function () {
          spaceDragging = true;
          spaceKey.classList.add("active-trackpad");
          editor.focus();
          if (navigator.vibrate) { try { navigator.vibrate(8); } catch (e2) {} }
        }, HOLD_MS);
      }, { passive: false });

      spaceKey.addEventListener("touchmove", function (e) {
        let t = null;
        for (let i = 0; i < e.touches.length; i++) {
          if (e.touches[i].identifier === spaceTouchId) { t = e.touches[i]; break; }
        }
        if (!t) return;
        e.preventDefault();
        ...
        // Relative drag: move the cursor by translated character/line
        // delta from the last touch point, using the caret's own current
        // screen position as the reference (finger doesn't need to be
        // over the editor at all).
        const rect = getCaretClientRect();
        if (!rect) return;
        const dx = t.clientX - spStartX;
        const dy = t.clientY - spStartY;
        moveCursorTo(rect.x + dx, rect.y + rect.height / 2 + dy);
        spStartX = t.clientX;
        spStartY = t.clientY;
      }, { passive: false });

      spaceKey.addEventListener("touchend", function (e) {
        e.preventDefault(); // stop the ghost click Safari fires after touchend
        const wasTap = !spaceDragging;
        endSpaceDrag();
        if (wasTap) {
          // Plain tap (no hold/drag) still types a space, same as before.
          const sel = editor.getSelection();
          editor.executeEdits("mobile-toolbar", [{ range: sel, text: " ", forceMoveMarkers: true }]);
          editor.focus();
        }
      }, { passive: false });

      spaceKey.addEventListener("touchcancel", endSpaceDrag, { passive: true });
    }
  }
